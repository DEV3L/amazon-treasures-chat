...
from src.exporters.about.persona_exporter import FILE_NAME, PersonaExporter
from src.exporters.exporter import DATA_DIR, DATA_FILE_PREFIX


@pytest.fixture(name="exporter")
def build_exporter():
    return PersonaExporter()


@patch("src.exporters.about.persona_exporter.create_dir")
@patch("src.exporters.about.persona_exporter.does_data_exist")
...
@patch("src.exporters.about.persona_exporter.shutil")
...
def test_get_file_path(exporter):
    ...
    assert result == f"bin/about/{DATA_FILE_PREFIX}_persona.txt"
